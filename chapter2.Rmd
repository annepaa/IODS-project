...
p
```
It can be seen from summary graphics that age range is 17-55 years, but there is more young people. The range of attitude, deep, stra and surf variables is roughly between 1 and 5 and exam points vary between 7-33 and the most frequent score is average, they seem to obey Gaussian curve.
...
p10
```
There is no clear result in exam points in terms of strategic learning dimension, the results in both men and women is scattered all over the picture.However, women seem to have lower exam points than men and there is slight curve upwards; with higher strategic learning scores student gets better exam points.
...
I will do a multiple regression analysis and choose first three variables as explanatory variables: attitude, strategic and surface learning. Exam points is a dependent variable.
...
```
The highest correlation is with exam point is with attitude, strategic and surface learning.
...
```
Residuals range is -17.6-10.9.
The estimated regression coefficient for attitude (3.47) is statistically significa
```nt (P-value <0.05) and is related to exam points. P-value of Stra's regression coefficient (0.91) is a bit over 0.05 (0.089). Standard errors for both attitude and stra are about the same, approx. 0.5. 
Multiple R-squared is 0.2048, so together attitude and strategic learning account almost 20% of the variation in exam points.

Let's now draw following diagnostic plots: Residuals vs Fitted values, Normal QQ-plot and Residuals vs Leverage.
With par(mfrow = c(2,2)) we can have all the wanted plots in the same graph into 3 rows and 1 columns.
